...
function CRC32Stream(var Stream: TFileStream; StartOffset: LongWord; EndOffset: LongWord; Initial: LongWord): Longword;
...
var
  ...
  I,C,Total,ReadCount: Integer;
  TempResult: Longword;
begin
  {Calculate the CRC32 image for Stream}
  TempResult := Initial;
  Total := Integer(EndOffset - StartOffset);
  Stream.Seek(StartOffset,0);
  C := 0;
  while (Stream.Position < EndOffset) do begin
    if C + 8192 > Total then
    ReadCount := Stream.Read(Buffer, Total - C)
    else
...
begin
  ...
  try
    Result := CRC32Stream(Stream, 0, Stream.Size, $FFFFFFFF);
  finally
    Stream.Free;
  end;
end;
